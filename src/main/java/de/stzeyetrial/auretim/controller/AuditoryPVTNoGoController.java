 ...
 * @author strasser
 */
public class AuditoryPVTNoGoController extends AbstractController {
	...
	public AuditoryPVTNoGoController() {
	...
	public void initialize(final URL location, final ResourceBundle resources) {
		_volumeTextField.textProperty().bindBidirectional(Config.getInstance().auditoryPVT_GO_NOGOvolumeProperty(), NumberFormat.getIntegerInstance());
	...
	private void start() {
		...
		final int frequency				= Config.getInstance().auditoryPVT_GO_NOGOfrequencyProperty().get();
		final int pulseDuration			= Config.getInstance().auditoryPVT_GO_NOGOpulseDurationProperty().get();
		final int minimumResponseTime	= Config.getInstance().auditoryPVT_GO_NOGOminimumResponseTimeProperty().get();
		final int minimumDelay			= Config.getInstance().auditoryPVT_GO_NOGOminimumDelayProperty().get();
		final int maximumDelay			= Config.getInstance().auditoryPVT_GO_NOGOmaximumDelayProperty().get();
		final int timeout				= Config.getInstance().auditoryPVT_GO_NOGOtimeoutProperty().get();
		final int repetitions			= Config.getInstance().auditoryPVT_GO_NOGOrepetitionsProperty().get();

		final List<Result> results = Session.getCurrentSession().getResults();
		results.clear();

		final AbstractRunnerTask task = new NoGoRunnerTask(results, frequency, Config.getInstance().auditoryPVT_GO_NOGOvolumeProperty(), pulseDuration, minimumResponseTime, minimumDelay, timeout, repetitions, false);
	...
	private void lowerVolume(final ActionEvent e) {
		final int volume = Config.getInstance().auditoryPVT_GO_NOGOvolumeProperty().get() - Config.VOLUME_DELTA;
		if (volume >= Config.MIN_VOLUME) {
			Config.getInstance().auditoryPVT_GO_NOGOvolumeProperty().set(volume);
		}
	}
